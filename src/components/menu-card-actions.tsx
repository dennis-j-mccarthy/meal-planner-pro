...
import { deleteMenuCard, sendBonAppetitStatus, acceptMenuCard } from "@/app/actions";
...
export function MenuCardActions({ menuCardId, accepted }: MenuCardActionsProps) {
  ...
  async function handleSend() {
    ...
    try {
      const res = await sendBonAppetitStatus(fd);
      if (res.ok) {
        setSendStatus("sent");
        setSendDetail(`Delivered to ${res.to}${res.id ? ` (id ${res.id.slice(0, 8)}…)` : ""}`);
      } else {
        setSendStatus("error");
        setSendDetail(res.error);
      }
    } catch (e) {
      setSendStatus("error");
      setSendDetail(e instanceof Error ? e.message : "Request failed");
    }
  }
  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      ...
      {/* Detailed send progress */}
      {sendStatus !== "idle" && (
        <p
          className={`max-w-xs text-right text-xs leading-snug break-words ${
            sendStatus === "sent"
              ? "text-green-600"
              : sendStatus === "error"
                ? "text-red-600"
        ...
        >
          {sendStatus === "sending" && "Rendering PDF & sending to Beth…"}
          {sendStatus === "sent" && `✓ ${sendDetail}`}
          {sendStatus === "error" && `Failed — ${sendDetail}`}
        </p>
      )}
    </div>
  );
