...
/** Estimate per-serving nutrition for a recipe from its title and ingredients. */
export async function generateRecipeNutrition(
  title: string,
  ...
  const prompt = [
    "You are a registered dietitian estimating nutrition for a recipe.",
    "Give a realistic estimate of the nutrition PER SERVING. If unsure, make a sensible professional estimate — do not refuse.",
    "",
    `Recipe: ${title}`,
    servings ? `Servings: ${servings}` : "",
    ingredientsText ? `Ingredients:\n${ingredientsText}` : "",
    "",
    "Return ONLY valid JSON with exactly these fields (per serving):",
    "{",
    '  "calories": number (kcal, integer),',
    '  "protein": "string with unit, e.g. 32 g",',
    '  "carbs": "string with unit, e.g. 41 g",',
    '  "fat": "string with unit, e.g. 18 g",',
    '  "fiber": "string with unit, e.g. 6 g",',
    '  "sodium": "string with unit, e.g. 540 mg"',
    "}",
  ]
    .filter(Boolean)
    .join("\n");
  ...
  const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!text) {
    throw new Error("No nutrition returned from Gemini");
  }
  ...
  return {
    ...
    protein: str(parsed.protein),
    carbs: str(parsed.carbs),
    fat: str(parsed.fat),
    fiber: str(parsed.fiber),
    sodium: str(parsed.sodium),
  };
}
