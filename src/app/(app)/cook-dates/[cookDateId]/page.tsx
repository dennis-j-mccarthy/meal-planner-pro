...
export default async function CookDateDetailPage({
  ...
  const hasRespondedPlan = cookDate.proposals.some((p) => p.status === "CLIENT_RESPONDED");
  const hasSentPlan = cookDate.proposals.some((p) => p.status === "SENT");
    ...
    <div className="space-y-6">
      ...
      <div className={`panel p-6 ${currentStep === 2 ? "ring-2 ring-[var(--accent)] ring-offset-2" : ""}`}>
        <div className="flex items-center gap-2 mb-3">
          ...
          {hasRespondedPlan && !isApproved && (
            <span className="ml-1 inline-flex items-center rounded-md bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-600/20">
              Client responded
            </span>
          )}
        </div>
        {isApproved ? (
          ...
          <p className="text-sm font-medium text-emerald-700">
            The client reviewed their menu — open the meal plan above to see their picks and approve.
          </p>
        ) : hasSentPlan ? (
          <p className="text-sm text-sky-700">
            Sent to the client — waiting for their response. They review it from a link (no login).
          </p>
        ) : hasMealPlan ? (
          <p className="text-sm text-slate-500">
            Open the meal plan above, then click &ldquo;Send to client&rdquo;.
          </p>
        ) : (
          <p className="text-sm text-slate-400">
